...
BEGIN
    INSERT INTO stock (product_id, shelf_location, stock_quantity)
    VALUES (product_id, shelf, quantity)
...
-- Procedur för att visa alla ordrar med totalsummor
...
-- Procedur för att visa alla ordrar
...
-- Procedur för att visa alla kunder
...
-- Procedur för att hämta en kund med ID
...
-- Procedur för att skapa en order
...
BEGIN
    ...
    VALUES (p_order_date, p_total_price, p_customer_id, p_order_status, NOW(), NOW());
END //
DELIMITER ;

-- Procedur för att visa orderdetaljer
...
BEGIN
    SELECT
        ...
        oi.product_id,
        p.product_name,
        oi.item_count AS total_product,
...
-- Procedur för att ändra orderstatus till 'bearbetas'
DROP PROCEDURE IF EXISTS p_change_order_status;
DELIMITER //
CREATE PROCEDURE p_change_order_status(IN p_order_id INT)
BEGIN
    UPDATE `order`
    SET `order_status` = 'bearbetas'
    WHERE `order_id` = p_order_id;
END //
DELIMITER ;

-- Procedur för att hämta orderinformation
DROP PROCEDURE IF EXISTS p_get_order_information;
DELIMITER //
CREATE PROCEDURE p_get_order_information(IN p_order_id INT)
BEGIN
    SELECT * FROM `order` WHERE `order_id` = p_order_id;
END //
DELIMITER ;

-- Procedur för att uppdatera orderstatus till 'slutförd'
DROP PROCEDURE IF EXISTS p_update_order_status_to_shipped;
DELIMITER //
CREATE PROCEDURE p_update_order_status_to_shipped(IN p_order_id INT)
BEGIN
    UPDATE `order`
    SET `order_status` = 'slutförd', `shipped` = NOW()
    WHERE `order_id` = p_order_id;
END //
DELIMITER ;

-- Procedur för mjuk radering av en order
DROP PROCEDURE IF EXISTS p_soft_delete_order;
DELIMITER //
CREATE PROCEDURE p_soft_delete_order(IN p_order_id INT)
BEGIN
    UPDATE `order`
    SET `order_status` = 'avbruten', `deleted` = NOW()
    WHERE `order_id` = p_order_id;
END //
DELIMITER ;

-- Procedur för att visa plocklista från lagret
...
BEGIN
    SELECT
        ...
        oi.product_id,
        p.product_name,
        oi.item_count AS order_quantity,
        (oi.price * oi.item_count) AS order_price,
        s.shelf_location,
        s.stock_quantity,
        (s.stock_quantity - oi.item_count) AS quantity_difference
    FROM
        `order_item` oi
    JOIN
...
-- Procedur för att hämta orderstatus
...
-- Procedur för att visa en order med totalsummor
DROP PROCEDURE IF EXISTS p_show_order_with_totals_custom;
DELIMITER //
CREATE PROCEDURE p_show_order_with_totals_custom(IN p_order_id INT)
BEGIN
    SELECT
        o.order_id,
        o.order_date,
